...
def evaluate(env, model):
    ...
    model.stepSequence = stepSequence        
    return
...
def parallel_evaluation(population, environments):
    threads = []
    chunk_size = len(environments)
    for i in range (0, len(population), chunk_size):
        current_models = population[i:i + chunk_size]
        threads = []
        for env, model in zip (environments, current_models):
            env.reset()
            thread = threading.Thread(target=evaluate, args = (env, model))
...
if __name__ == "__main__":
    ...
    for i in range (params["generations"]):
        ...
        parallel_evaluation(population, environments)
        # for model in population:
        #     env.reset()
        #     model.fitness, model.action_sequence, newSeed = evaluate(env, model)
        #     fitness_list.append(model.fitness)
