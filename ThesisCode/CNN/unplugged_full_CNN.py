...
configpath = os.path.join(os.path.dirname(__file__), "unplugged_config_full.json")
...
def save(agent, fitness_values = None, starting_point = None):
    ...
    with open(f"sav/{hostname}_full_CNN_agent_state_gen{agent.generation}_f{agent.fitness}_sigma{params['use_sigma']}.json", "w") as file:
